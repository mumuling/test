...
public class FolderAdapter extends BaseAdapter {
        ...
        if (holder != null) {
            if (i == 0) {
                ...
                if (mFolders.size() > 0) {
                    ...
                    if (f != null) {
                        ImageLoader imageLoader = new ImageLoader.Builder().url("file://" + f.cover.path)
                                .placeHolder(R.color.black)
                                .imgView(holder.cover).size(50, 50).build();
    ...
    class ViewHolder {
        ...
        void bindData(Folder data) {
            ...
            if (data.cover != null) {
                ImageLoader imageLoader = new ImageLoader.Builder().url("file://" + data.cover.path).placeHolder(R.color.black).
                        imgView(cover).size(50, 50).build();
                ImageLoaderUtil.getInstance().loadImage(mContext, imageLoader);
            } else {
